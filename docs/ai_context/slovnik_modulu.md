...
| **Legal Spain (check-in / SES)** | Záložka index 7 (za Nastavením ve stacku, v menu za Rezervacemi). Viditelná jen při aktivním `legal_spain`. U rezervace hosté + odkaz `/checkin/:token`. WS hesla majitel nevidí. | `legalComplianceDashboardProvider`, `reservationGuestsProvider` |
