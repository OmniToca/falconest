...
| **Legal Spain (check-in / SES)** | Záložka index 7 (za Nastavením ve stacku, v menu za Rezervacemi). Viditelná jen při aktivním `legal_spain`. U rezervace hosté + odkaz `/checkin/:token`. WS hesla majitel nevidí. UI: headline + TabBar v těle, `premiumCardShell` jako Automatizace/Komunikace. | `legalComplianceDashboardProvider`, `reservationGuestsProvider` |
...
## 15b. Legal Spain – check-in a SES (`lib/features/legal_spain`)

Placený modul **`legal_spain`** (RD 933/2021, Valencijsko / SES Hospedajes). Agentura platí `per_apartment`; veřejný `/checkin/:token` i SOAP jen když `is_legal_spain_module_active`.

| Blok | Popis | Providery |
|------|--------|-----------|
| **Fáze 0** | Arrendador = majitel nebo agentura (kódy z sede Interior). PV vždy; RH jen Direct/Other. GDPR 3 roky. | — |
| **Hosté** | `reservation_guests` + podpis 14+. Dvě cesty: in-app (admin/owner/worker) nebo veřejný formulář. | `reservationGuestsProvider`, `guestCheckinProvider` |
| **SES SOAP** | Edge `ses-hospedajes`: alta → lote → `consultaLote`. Stavy + alert (notifikace) při reject/timeout/SLA 22:00 Madrid. | `legalComplianceDashboardProvider` |
| **Kniha** | Export PDF z `reservation_guests`. | `legalVisitorBookProvider` |
...
- **Owner portal view – server-side read-only (2026-09-13):** migrace **`20260913140000_owner_portal_view_readonly_guard.sql`** – aktivní session v **`owner_portal_view_sessions`** blokuje INSERT/UPDATE přes RESTRICTIVE RLS (tasks, disposition, offset proposals) + trigger u proposals; RPC **`close_my_open_owner_portal_view_sessions`** při loginu admin/manager a před novým náhledem (orphan po F5).
- **`lib/core/utils/app_logger.dart`** – v release (non-web) forward do **Firebase Crashlytics**; init v **`main.dart`** + `FlutterError.onError` / `PlatformDispatcher.onError`.
- **`lib/core/offline/smart_merge_rules.dart`** – čistá Timestamp Merging / Smart Merge pravidla (unit testy); používá **`WorkerSyncService`** (mobile).
- **`admin_tasks_screen.dart`** – dialogy a form sekce v `part` souborech pod **`widgets/admin_tasks_*.dart`** (recalculate, add, edit, form sections).
- **Web deferred HQ:** `app_router` načítá Super Admin přes **`super_admin_deferred.dart`** + **`DeferredPage`**.
- **Backlog – Verifacti / Veri\*Factu (2026-09-13):** plánované napojení API ([`GET /verifactu/declaracion`](https://www.verifacti.com/precios#/paths/~1verifactu~1declaracion/get) + fakturační tok AEAT). Roadmapa: **P3.5** v `docs/audits/ENGINEERING_STABILIZATION_ROADMAP.md` a Fáze 3 v `ROADMAP_STABILIZATION.md`.
