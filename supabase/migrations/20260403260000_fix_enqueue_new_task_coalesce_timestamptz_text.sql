-- =============================================================================
-- FalcoNest – oprava COALESCE(timestamptz, text) v enqueue_internal_push_on_new_task_assignment
-- =============================================================================
-- PROČ: tasks.scheduled_start je timestamptz, tasks.due_date je text (ISO z aplikace).
-- COALESCE musí mít argumenty stejného typu — jinak PostgreSQL hlásí
-- "COALESCE types timestamp with time zone and text cannot be matched".
-- Řešení: due_date nejdřív na text (trim, prázdný řetězec → NULL), pak ::timestamptz.
-- =============================================================================
...
BEGIN
  ...
  when_ts := COALESCE(
    NEW.scheduled_start,
    NULLIF(btrim(NEW.due_date::text), '')::timestamptz
  );
