-- =============================================================================
-- FalcoNest – notifications.metadata + in-app (web) u nového úkolu
-- =============================================================================
-- 1) Sloupec metadata (jsonb) pro proklik – např. {"task_id": "<uuid>"}.
-- 2) Rozšíření enqueue_internal_push_on_new_task_assignment: při new_task_assigned_web
--    INSERT do public.notifications (zvoneček), type = 'new_task'.
-- =============================================================================
...
COMMENT ON COLUMN public.notifications.metadata IS
  'Volitelná data pro UI (např. task_id pro navigaci na detail úkolu).';

-- Kompletní funkce: push fronta + e-mail fronta + in-app notifikace
...
BEGIN
  ...
  SELECT
    np.new_task_assigned_push,
    np.new_task_assigned_email,
    np.new_task_assigned_web
  INTO prefs_push, prefs_email, prefs_web
  FROM public.notification_preferences np
  WHERE np.profile_id = NEW.assigned_to;

  prefs_push := COALESCE(prefs_push, true);
  prefs_email := COALESCE(prefs_email, true);
  prefs_web := COALESCE(prefs_web, true);
  ...
  IF prefs_push IS NOT TRUE
     AND NOT (prefs_email IS TRUE AND worker_email IS NOT NULL)
     AND prefs_web IS NOT TRUE THEN
    RETURN NEW;
  END IF;
...
COMMENT ON FUNCTION public.enqueue_internal_push_on_new_task_assignment() IS
  'Po INSERT/UPDATE assigned_to: internal_push (new_task_assigned_push), e-mail fronta (email + profiles.email), in-app zvoneček (new_task_assigned_web → notifications, type new_task, metadata.task_id). SECURITY DEFINER.';
